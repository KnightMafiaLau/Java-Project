...
public class Book {
	...
	private void setTitleAndAuthor() {
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			Matcher authorMatcher = authorPattern.matcher(lines.get(i));
			Matcher titleMatcher = titlePattern.matcher(lines.get(i));
			boolean checkAuthor = authorMatcher.lookingAt();
			boolean checkTitle = titleMatcher.lookingAt();
			
			if (checkTitle) {
				int titleIndex = (line.indexOf(":"));
				String Title = new String();
				Title = line.substring(titleIndex+1);
				this.title = Title.strip();
				
			}else if (checkAuthor){
				int authorIndex = (line.indexOf(":"));
				String Author = new String();
				Author = line.substring(authorIndex+1);
	...
	private void countWords() {
		...
		for(int i = 0; i < lines.size(); i++){
		   ...
		   for(int j = 0; j < temp.length(); j++){
		      if(!valid.contains(temp.substring(j,j+1))){
		   ...
		   for(int z = 0; z < arrOfTemp.length; z++){
		      if(this.wordCounts.containsKey(arrOfTemp[z])){
		         int v = this.wordCounts.get(arrOfTemp[z]);
		         this.wordCounts.put(arrOfTemp[z], v+1);
		      }
		      else{
		         this.wordCounts.put(arrOfTemp[z], 1);
		      }
		   }
		   
		   this.wordCount += arrOfTemp.length;
		}
	}
